...
\textbf{Hindsight}~\cite{hindsight2025} achieves 91.4\% via selective
memory formation (open-source backbone, model unspecified in paper).
...
\begin{itemize}[leftmargin=*, itemsep=1pt]
  \item \textbf{Truncation} (primary baseline): Last 32,000 characters of
    ...
    the context window.
  \item \textbf{RAG} (embedding retrieval baseline): \texttt{text-embedding-3-small}
    embeddings for all turns; cosine similarity retrieval of top-20 turns
    ($\pm$1 context-window expansion); \texttt{gpt-4o-mini} for answering.
    Same 100 class-balanced samples, same scorer as \rlm{}.
...
\texttt{gpt-4o-mini}). Truncation window: 32K chars. \rlm{} processes
each session independently; the full ${\sim}490$K-char history is never
loaded into one call. RAG: top-20 turns by cosine similarity, same samples.}
\label{tab:reallme}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
...
RAG top-20 (ours)             & 0.430 & 0.196 & ${\sim}$8K   & 7.5\,s \\
\textbf{\rlm{} (ours)} & \textbf{0.460} & \textbf{0.429} &
\textbf{37,216} & \textbf{${\sim}$4\,s}$^{\ddagger}$ \\
\midrule
...
\multicolumn{5}{l}{\scriptsize $^\ddagger$Parallel \texttt{llm\_query\_parallel()}
(\texttt{ThreadPoolExecutor}); sequential baseline: 221\,s.}
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{EM and F1 by question type on real LongMemEval-S (100 samples).
RAG: top-20 cosine-similarity retrieval, same samples and scorer.
Truncation is the primary scalable baseline.}
\label{tab:reallme_bytype}
\small
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{Category} & \textbf{n} & \textbf{\rlm{} EM} &
\textbf{\rlm{} F1} & \textbf{RAG EM} & \textbf{Trunc.\ EM} \\
\midrule
single-session-user      & 16 & \textbf{0.875} & \textbf{0.743} & 0.750          & 0.000 \\
single-session-assistant & 17 & 0.647          & \textbf{0.620} & 0.706          & 0.059 \\
knowledge-update         & 16 & \textbf{0.625} & 0.394          & 0.563          & 0.188 \\
temporal-reasoning       & 17 & \textbf{0.412} & \textbf{0.475} & 0.176          & 0.059 \\
multi-session            & 18 & 0.222          & 0.242          & \textbf{0.389} & 0.000 \\
single-session-preference & 16 & 0.000         & 0.111          & 0.000          & 0.000 \\
\midrule
\textbf{Overall}         & 100 & \textbf{0.460} & \textbf{0.429} & 0.430 & 0.050 \\
\bottomrule
\end{tabular}
\end{table}
...
Hindsight~\cite{hindsight2025}   & OS 20B+       & 91.4\% & Selective formation  & \yes & \yes \\
...
RAG top-20 (ours)        & gpt-4o-mini & 43.0\% & Embed.\ retrieval & \no & \yes \\
...
\paragraph{RAG vs.\ \rlm{}.}
The RAG baseline achieves 43.0\% EM --- only 3 percentage points below
\rlm{} --- but its F1 is 19.6\% versus \rlm{}'s 42.9\%.
This gap reveals fundamentally different answer behaviours: RAG retrieves
turn-level excerpts and generates verbose summaries that partially overlap
with the gold string but rarely match it precisely; \rlm{}'s programmatic
approach distils exact values from sub-agent findings.
RAG \emph{surpasses} \rlm{} on multi-session aggregation (38.9\% vs.\
22.2\% EM): semantic similarity retrieves relevant turns from multiple
sessions when the question vocabulary overlaps the answer.
However, RAG lags substantially on temporal-reasoning (17.6\% vs.\ 41.2\%),
where questions require computing date differences between turns ---
a computation that embedding similarity cannot perform but \rlm{}'s
Python-equipped LLM can.
RAG is also efficient: ${\sim}$8K tokens per query versus ${\sim}$37K for
\rlm{}, at the cost of lower F1 precision.
...
\paragraph{Latency.}
Sequential sub-agent calls average ${\sim}221$\,s per query (serial
${\sim}50$ sessions $\times$ ${\sim}4$\,s each). Sub-agent calls are
embarrassingly parallel: \texttt{llm\_query\_parallel()} fans all sessions
out via \texttt{ThreadPoolExecutor}, bounding wall time to the slowest
single call. Verified on 10 sessions: ${\sim}4$\,s per query, $54\times$
faster than sequential, with no accuracy loss (5/5 correct). At
gpt-4o-mini prices, cost is ${\approx}\$0.005$ per query.
...
\begin{enumerate}[leftmargin=*, itemsep=2pt]
  ...
  \item \textbf{Latency.} Sequential sub-agent calls average ${\sim}221$\,s
    per query. With \texttt{llm\_query\_parallel()} (implemented;
    \texttt{ThreadPoolExecutor}, \texttt{max\_workers=20}), all sessions
    fire simultaneously and wall time drops to ${\sim}4$\,s --- a $54\times$
    reduction, verified on 10 sessions with 5/5 accuracy. Production
    deployments should use the parallel path.
...
Parallel sub-agent execution (\texttt{llm\_query\_parallel}) is implemented
and reduces latency from ${\sim}221$\,s to ${\sim}4$\,s ($54\times$),
making \rlm{} production-viable. The remaining gap versus trained systems
(46\% vs.\ 85--95\%) is addressable via semantic search, recency-weighted
retrieval, and LLM-as-judge evaluation for preference questions --- all
without training, preserving the zero-training, infinite-history property.
...
# RAG baseline (same 100 samples, seed=42)
